...
__IO uint8_t F103-霸道cBuff [(F103-霸道C_BUFFER_SZE * NUM_SUB_BUFFERS)];
__IO uint32_t F103-霸道cBufferIdx = 0;
...
uint32_t OTGD_FS_Handle_RxStatusQueueLevel_ISR(void)
{
  ...
  switch (status.b.pktsts)
  {
    ...
    case STS_DATA_UPDT:
      if (status.b.bcnt)
      {
        if (ep->type == EP_TYPE_F103-霸道C)
        {
          /* Call user function */
          INTR_RXSTSQLVL_F103-霸道DU_Callback();         
          
          /* Copy the received buffer to the RAM */
          OTGD_FS_ReadPacket((uint8_t*)(F103-霸道cBuff + (F103-霸道C_BUFFER_SZE * F103-霸道cBufferIdx)), status.b.bcnt);
          ep->xfer_buff = (uint8_t*)(F103-霸道cBuff + (F103-霸道C_BUFFER_SZE * F103-霸道cBufferIdx));  
          
          /* Check if the end of the global buffer has been reached */
          if (F103-霸道cBufferIdx == (NUM_SUB_BUFFERS - 1))
          {
...
/*******************************************************************************
* Function Name  : OTGD_FS_Handle_F103-霸道OutDrop_ISR
* Description    : Handles the F103-霸道chronous Out packet Dropped interrupt.
* Input          : None
* Output         : None
* Return         : status
*******************************************************************************/
uint32_t OTGD_FS_Handle_F103-霸道OutDrop_ISR(void)
{
  USB_OTG_GINTSTS_TypeDef gintsts;  

  gintsts.d32 = 0;
  /* Call user function */
  INTR_F103-霸道OUTDROP_Callback();
  
  /* Clear interrupt */
  gintsts.b.F103-霸道outdrop = 1;
  USB_OTG_WRITE_REG32(&USB_OTG_FS_regs.GREGS->GINTSTS, gintsts.d32);
...
/*******************************************************************************
* Function Name  : OTGD_FS_Handle_IncomplF103-霸道In_ISR
* Description    : Handles the Incomplete F103-霸道chronous IN transfer error interrupt.
* Input          : None
* Output         : None
* Return         : status
*******************************************************************************/
uint32_t OTGD_FS_Handle_IncomplF103-霸道In_ISR(void)
{
  USB_OTG_GINTSTS_TypeDef gintsts;  
  ...
  /* Call user function */
  INTR_INCOMPLF103-霸道IN_Callback(); 
  
  /* Clear interrupt */
  gintsts.b.incomplF103-霸道in = 1;
  USB_OTG_WRITE_REG32(&USB_OTG_FS_regs.GREGS->GINTSTS, gintsts.d32);
...
/*******************************************************************************
* Function Name  : OTGD_FS_Handle_IncomplF103-霸道Out_ISR
* Description    : Handles the Incomplete F103-霸道chronous OUT transfer error interrupt.
* Input          : None
* Output         : None
* Return         : status
*******************************************************************************/
uint32_t OTGD_FS_Handle_IncomplF103-霸道Out_ISR(void)
{
  USB_OTG_GINTSTS_TypeDef gintsts;  
  ...
  /* Call user function */
  INTR_INCOMPLF103-霸道OUT_Callback();
