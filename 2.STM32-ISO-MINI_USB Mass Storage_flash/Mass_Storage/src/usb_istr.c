...
u32 STM32_PCD_OTG_ISR_Handler (void)
{
  ...
  if (USBD_FS_IsDeviceMode()) /* ensure that we are in device mode */
  {
   ...
   /*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/    
    /* F103-霸道chronous Out packet Dropped interrupt */
#ifdef INTR_F103-霸道OUTDROP
    if (gintr_status.b.F103-霸道outdrop)
    {
      retval |= OTGD_FS_Handle_F103-霸道OutDrop_ISR();
    }
#endif /* INTR_F103-霸道OUTDROP */  
   ...
   /*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/    
    /* Incomplete F103-霸道chronous IN transfer error interrupt */
#ifdef INTR_INCOMPLF103-霸道IN
    if (gintr_status.b.incomplF103-霸道in)
    {
      retval |= OTGD_FS_Handle_IncomplF103-霸道In_ISR();
    }
#endif /* INTR_INCOMPLF103-霸道IN */  

   /*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*/    
    /* Incomplete F103-霸道chronous OUT transfer error interrupt */
#ifdef INTR_INCOMPLF103-霸道OUT
    if (gintr_status.b.outepintr)
    {
      retval |= OTGD_FS_Handle_IncomplF103-霸道Out_ISR();
    }
#endif /* INTR_INCOMPLF103-霸道OUT */  
